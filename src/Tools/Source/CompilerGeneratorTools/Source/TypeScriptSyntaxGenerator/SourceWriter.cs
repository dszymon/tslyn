...
namespace TypeScriptSyntaxGenerator
{
    internal class SourceWriter : AbstractFileWriter
    {
        ...
        private void WriteInternal()
        {
            ...
            WriteLine("namespace Microsoft.CodeAnalysis.TypeScript.Syntax.InternalSyntax;");
        ...
        {
            WriteFileHeader();
            WriteLine("namespace Microsoft.CodeAnalysis.TypeScript.Syntax;");
            this.WriteRedTypes();
        }
        ...
        {
            WriteFileHeader();
            WriteLine("namespace Microsoft.CodeAnalysis.TypeScript;");
            WriteLine("using System.Diagnostics.CodeAnalysis;");
            WriteLine("using Microsoft.CodeAnalysis.TypeScript.Syntax;");
        ...
        private void WriteGreenType(TreeType node)
        {
            ...
            else if (node is Node)
            {
                ...
                foreach (var field in nodeFields)
                {
                    ...
                    {
                        var type = $"CoreSyntax.{field.Type}";
                        WriteLine($"public {OverrideOrNewModifier(field)}{type} {field.Name} => new {type}(new CoreSyntax.SyntaxList<TypeScriptSyntaxNode>(this.{CamelCase(field.Name)}));");
                    }
                    else if (field.Type == "SyntaxNodeOrTokenList")
                    {
                        var type = $"CoreSyntax.SyntaxList<TypeScriptSyntaxNode>";
                        WriteLine($"public {OverrideOrNewModifier(field)}{type} {field.Name} => new {type}(this.{CamelCase(field.Name)});");
                    }
                    else
                    {
                ...
                WriteLine();
                WriteLine($"internal override SyntaxNode CreateRed(SyntaxNode? parent, int position) => new TypeScript.Syntax.{node.Name}(this, parent, position);");
        ...
        {
            WriteLine();
            WriteLine($"public override void Accept(TypeScriptSyntaxVisitor visitor) => visitor.Visit{StripPost(node.Name, "Syntax")}(this);");
            WriteLine($"public override TResult Accept<TResult>(TypeScriptSyntaxVisitor<TResult> visitor) => visitor.Visit{StripPost(node.Name, "Syntax")}(this);");
        }
        ...
        private void WriteGreenVisitor(bool withResult)
        {
            ...
            WriteLine();
            WriteLine("internal partial class TypeScriptSyntaxVisitor" + (withResult ? "<TResult>" : ""));
            OpenBlock();
        ...
        private void WriteGreenUpdateMethod(Node node)
        {
            ...
            {
                var type =
                    f.Type == "SyntaxNodeOrTokenList" ? "CoreSyntax.SyntaxList<TypeScriptSyntaxNode>" :
        ...
        private void WriteGreenRewriter()
        {
            ...
            WriteLine();
            WriteLine("internal partial class TypeScriptSyntaxRewriter : TypeScriptSyntaxVisitor<TypeScriptSyntaxNode>");
            OpenBlock();
            int nWritten = 0;
            foreach (var node in nodes.OfType<Node>())
            {
                ...
                if (nWritten > 0)
                    WriteLine();
                nWritten++;
                WriteLine($"public override TypeScriptSyntaxNode Visit{StripPost(node.Name, "Syntax")}({node.Name} node)");
                Indent();
        ...
        private void WriteGreenFactory(Node nd, bool withSyntaxFactoryContext = false)
        {
                ...
                valueFields.Count + nodeFields.Count <= 3)
            {
                ...
                {
                    Write("var cached = TypeScriptSyntaxNodeCache.TryGetNode((int)");
                }
                else
                {
        ...
        private void WriteGreenFactoryParameters(Node nd)
        {
            Write(CommaJoin(
                ...
                nd.Fields.Select(f =>
                {
                    ...
                    {
                        "SyntaxNodeOrTokenList" => "CoreSyntax.SyntaxList<TypeScriptSyntaxNode>",
        ...
        private void WriteRedType(TreeType node)
        {
            ...
            if (node is AbstractNode)
            {
                ...
                OpenBlock();
                WriteLine($"internal {node.Name}(InternalSyntax.TypeScriptSyntaxNode green, SyntaxNode? parent, int position)");
        ...
        private void WriteRedAcceptMethod(Node node, bool genericResult)
        {
            ...
            WriteLine($"public override {(genericResult ? "TResult?" : "void")} Accept{genericArgs}(TypeScriptSyntaxVisitor{genericArgs} visitor){(genericResult ? " where TResult : default" : "")} => visitor.Visit{StripPost(node.Name, "Syntax")}(this);");
        }
        ...
        private void WriteRedVisitor(bool genericResult)
        {
            ...
            WriteLine();
            WriteLine("public partial class TypeScriptSyntaxVisitor" + genericArgs);
            OpenBlock();
            int nWritten = 0;
            foreach (var node in nodes.OfType<Node>())
            {
        ...
        private void WriteRedUpdateMethod(Node node)
        {
            ...
            if (nCompared > 0)
            {
                ...
                WriteLine(");");
                WriteLine("var annotations = GetAnnotations();");
                WriteLine($"return annotations?.Length > 0 ? ({node.Name})newNode.WithAnnotations(annotations) : newNode;");
                CloseBlock();
            }
        ...
        private void WriteRedRewriter()
        {
            ...
            WriteLine();
            WriteLine("public partial class TypeScriptSyntaxRewriter : TypeScriptSyntaxVisitor<SyntaxNode?>");
            OpenBlock();
            ...
            Write(CommaJoin(
                ...
                nodeFields.Select(f =>
                {
                    if (f.Type == "SyntaxToken")
                    {
                        ...
                        return $"{CamelCase(f.Name)}.Node.ToGreenList<Syntax.InternalSyntax.TypeScriptSyntaxNode>()";
                    else if (IsOptional(f))
