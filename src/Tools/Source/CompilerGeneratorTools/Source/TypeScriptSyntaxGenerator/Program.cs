...
// using TypeScriptSyntaxGenerator.Grammar;

namespace TypeScriptSyntaxGenerator
{
    public static class Program
    {
        ...
        private static int WriteGrammarFile(string inputFile, string outputLocation)
        {
            try
            {
                ...
                var outputMainFile = Path.Combine(outputLocation.Trim('"'), $"TypeScript.Generated.g4");

                using var outFile = new StreamWriter(File.Open(outputMainFile, FileMode.Create), Encoding.UTF8);
                outFile.Write(grammarText);
                */
                Console.WriteLine("Grammar generation is not supported yet.");
            }
            catch (Exception ex)
            {
